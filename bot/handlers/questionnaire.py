...
async def end_current_questionnaire_and_proceed(bot: Bot, chat_id: int, message_id: int, state: FSMContext, session: AsyncSession):
    ...
    if pending:
        await questionnaire_service.start_questionnaire(bot, chat_id, message_id, state, session)
    else:
...
@router.callback_query(QuestionnaireState.in_questionnaire, F.data.startswith("q_"))
